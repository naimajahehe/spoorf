...
- **`POST /api/dhcp/wakeup`**
  ...
  - **Validasi**: controller IP, gateway, dan CIDR wajib berada pada topologi RFC 1918 yang sama. Gagal → `400`.
  - **Zero delivery**: bila tidak ada satu pun datagram discovery berhasil dikirim → `503`.
  - **Response `200 OK`**:
    ```json
    {
      "success": true,
...
- **`POST /api/network/profile-refresh`** (Node `:5000` — kanonik) & **`POST /api/network/quick-reauth`** (alias usang/kompatibilitas)
