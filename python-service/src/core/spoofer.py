...
class ARPSpoofer:
    ...
    def micro_cut_batch(self, targets: List[Dict[str, Any]], hold_seconds: float = 1.5) -> Dict[str, Any]:
        """
        Quick Re-Auth Profiling: micro-cut SERENTAK ke sekumpulan target Unknown lalu restore.
        Tujuan: memancing OS target masuk transisi jaringan & mengirim DHCP REQUEST baru
        (ditangkap sniffer DHCP pasif) sehingga Option 12/55/60 (nama & OS) terisi.

        Reuse start()/stop() yang sudah tervalidasi (RFC1918/gateway/self) & punya restore ARP.
        Mulai semua sesi berturut-cepat (tiap sesi mem-burst di thread-nya = efektif serentak),
        tahan `hold_seconds`, lalu stop semua (restore). Dibatasi 16 target simultan agar tidak
        menabrak buffer driver Npcap; sisanya diproses gelombang berikutnya.
        """
        hold = max(0.5, min(3.0, float(hold_seconds)))
        MAX_SIMULTANEOUS = 16
        cut_count = 0
        errors: List[Dict[str, str]] = []

        # Pecah menjadi gelombang bila target > batas simultan
        waves = [targets[i:i + MAX_SIMULTANEOUS] for i in range(0, len(targets), MAX_SIMULTANEOUS)]
        for wave in waves:
            ...
            for t in wave:
                try:
                    sid = self.start(
                        ...
                        victim_mac=t.get('victim_mac'),
                        gateway_ip=t.get('gateway_ip'),
                        gateway_mac=t.get('gateway_mac'),
                        speed_limit=0,
                        victim_ipv6=t.get('victim_ipv6'),
                        gateway_ipv6=t.get('gateway_ipv6')
                    )
                    session_ids.append(sid)
                    cut_count += 1
                except Exception as e:
                    errors.append({'ip': str(t.get('victim_ip')), 'error': str(e)})

            if session_ids:
                logger.info(f"⚡ [Quick Re-Auth] Micro-cut SERENTAK {len(session_ids)} target selama {hold:.1f}s…")
                time.sleep(hold)
                for sid in session_ids:
                    try:
                        self.stop(sid)
                    except Exception as e:
                        logger.debug(f"Notice restoring micro-cut session {sid}: {e}")

        logger.info(f"✅ [Quick Re-Auth] Selesai. {cut_count} target dipancing re-auth, {len(errors)} dilewati.")
        return {'cut_count': cut_count, 'errors': errors}
