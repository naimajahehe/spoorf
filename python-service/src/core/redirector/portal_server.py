...
class PortalRequestHandler(BaseHTTPRequestHandler):
    ...
    def _render_landing_html(self, target_url: str, username: str) -> bytes:
        ...
        deep_link = f"instagram://user?username={urllib.parse.quote(username)}" if username else target_url
...
<head>
    ...
    <title>NetCut Sentinel - Redirecting to {display_user}</title>
    <style>
...
<body>
    <div class="card">
        ...
        <p>Akses internet pada perangkat Anda saat ini sedang dialihkan ke akun <strong>{display_user}</strong>.</p>
        
        <div class="btn-group">
            <a href="{deep_link}" class="btn btn-primary">Buka di Aplikasi Instagram</a>
            <a href="{target_url}" class="btn btn-secondary">Buka Lewat Browser Web</a>
        </div>
...
class CaptivePortalServer:
    def __init__(self, port: int = 80, redirect_url: str = "https://www.instagram.com/", instagram_username: str = ""):
        self.port = port
    ...
    def update_target(self, redirect_url: str, instagram_username: str = ""):
        with self._lock:
            self.redirect_url = redirect_url
            self.instagram_username = instagram_username
            PortalRequestHandler.redirect_url = redirect_url
